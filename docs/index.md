...
A funnel next to the search box opens a filter popup: annotation colour/type/comment, attachment and item type, *Has DOI / URL / Related / Links*, and multi-select tag, publication, author, and collection facets — with include/exclude, OR groups, and a removable chip bar above the items tree. A matching funnel in the reader filters the open document's annotations. A second funnel beside the reader sidebar's search box filters the **annotations pane** the same way — colour, type, tag, comment, person, date — while the document keeps every annotation, so ink strokes can leave the pane and stay on the page. Per document, or as a default for all of them (Use as Default replaces the default, Alt+click adds to it); it replaces Zotero's own selector at the foot of the pane (which you can keep in Settings). The funnel itself can be switched off there; the pane then follows the types ticked in Settings. Two marks: a green line under the funnel, and under the chips the default sets, says a default other than "show everything" exists (those chips are shown in every document, even one with nothing of that kind); a blue dot on the funnel says this document departs from the default, with its own filter on top of it or after Clear. The footer names the default wherever the document departs from it.
...
Weavero never writes to or modifies your PDF files or attachments — it only layers UI on top of Zotero's standard views and keeps its own data in separate files. It is developed and tested against Zotero 10.0-beta; a few features rely on Zotero 10 APIs and may be unavailable on Zotero 9. See the [README](https://github.com/mjthoraval/Weavero#readme) for full feature documentation, the [filtering rules](https://github.com/mjthoraval/Weavero/blob/main/docs/filter-rules.md) for the filter logic, [compatibility.md](compatibility.md) for feature-by-version status on Zotero 9 vs 10, **[Developing Zotero plugins with AI](developing-with-ai)** for the from-scratch guide, the Weavero workflow, and ecosystem resources (the workflow's instruction files themselves are [published in the repo](https://github.com/mjthoraval/Weavero/tree/main/.claude)), and [TESTING.md](https://github.com/mjthoraval/Weavero/blob/main/TESTING.md) for how Weavero is tested (including the [performance protocol](https://github.com/mjthoraval/Weavero/tree/main/bench) and the [Windows taskbar overlay findings](taskbar-overlay-testing.md), which may interest other plugin developers).
