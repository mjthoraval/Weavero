...
A funnel next to the search box opens a filter popup: annotation colour/type/comment, attachment and item type, *Has DOI / URL / Related / Links*, and multi-select tag, publication, author, and collection facets — with include/exclude, OR groups, and a removable chip bar above the items tree. A matching funnel in the reader filters the open document's annotations. A second funnel beside the reader sidebar's search box filters the **annotations pane** the same way — colour, type, tag, comment, person, date — while the document keeps every annotation, so ink strokes can leave the pane and stay on the page. Per document, or as a default for all of them (Use as Default replaces the default, Alt+click adds to it); it replaces Zotero's own selector at the foot of the pane (which you can keep in Settings). One mark, one meaning: a green line under the funnel, and under the chips the default sets, says the document follows a default other than "show everything" (those chips are shown even when the document has nothing of that kind); a blue dot on the funnel says the document departs from the default, with its own filter or after Clear. A departing document shows no green and names the default in the footer instead.
