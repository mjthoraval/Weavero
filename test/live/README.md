...
| `search-modes.js` | Quick-search MODE — every scope-menu mode, read from Zotero's own `_searchModes` (titleCreatorYear / fields / everything on 10.0.2) — × a Weavero chip × both apply orders, against DB-computed ground truth; then every mode→mode switch through the scope MENU (menuitem `doCommand()`) with a term and chip live, against the new mode's ground truth; advanced search included | `Zotero._wvModes` |
